...
配置数据往往是一个应用中最重要的数据之一，它关系到应用运行的每个阶段。而在分布式应用中，配置数据则又比单机应用增加里诸如远程服务的相关信息、节点的状态信息、Master 节点等数据。而且在如何存储和管理这些数据上，分布式应用要比单机应用复杂许多。因为分布式应用不能存在单点，而配置数据这样的重要数据必须分布式的存储，所以保证这些分布式的配置数据的一致性则成为重中之重。有的场景，例如一个应用节点向配置服务器注册它所能提供的服务的相关信息，例如 IP 地址、端口、服务版本等。这样的操作相对不容易产生冲突。但有的场景，例如为整个集群选举一个 Master 节点，因为在同一时刻，会有多个普通节点去竞选成为 Master 节点，所以冲突在所难免。此时，一个可以高效解决数据一致性问题的算法便是非常必要的了。
...
Paxos 算法的语言表述看上去不难，但是其中的技术难点并不少。好在现在已经有了很多的解决方案，其中最为著名的便是 Apache Zookeeper。
