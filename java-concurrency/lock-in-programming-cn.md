...
强一致性在多数情况下，即便是单机应用，往往也是难以实现的。分布式锁虽然能实现分布式系统的强一致性，但是代价过大，在多数的操作中不会使用。所以，现在的服务器端应用多数保证的是最终一致性。在分布式系统中，还有一个更麻烦的分布式一致性问题。解决这类问题就需要例如 PAXOS 这样的分布式一致性算法解决。
...
如果分布式存储，需要锁住A所在的数据和B所在的数据。如果是有数据镜像，需要保证多份数据的一致性。前者需要分布式事务或分布式锁。后者需要实现分布式一致性。

小范围分布式一致性，总体上通过消息队列记录操作日志实现最终一致性。
